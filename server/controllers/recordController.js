...
import { findRecordsByType, findRecord, findRecords } from '../helpers/finders';
...
export const getRedFlags = async (req, res) => {
  ...
  sendSuccess(res, 200, { records: result });
};
...
export const updateARecord = async (req, res) => {
  ...
  const record = await findRecord(res, req.params.recordID, false, req.payload.id);
...
export const updateStatus = async (req, res) => {
  ...
  const { id, isAdmin } = req.payload;
  const { recordID } = req.params;
  await findRecord(res, recordID, isAdmin, id);
...
export const deleteARecord = async (req, res) => {
  const record = await findRecord(res, req.params.recordID, req.payload.isAdmin, req.payload.id);
  if (record) {
  ...
  } else sendError(res, 404, 'Record not found');
};
